class grafo_r:
    ...
    def freq_gradi(self):
        ...
        return Counter(list(self.dict_gradi().values()))
    def freq_col(self):
        from collections import Counter
        return Counter(list(self._col.values()))  #frequenza gradi
    def omo(self):
        archi=[]
        archi_omo=0
        for n in self._nodi:
    ...
    def random_coloring(self,ripetizioni=1):
        ...
        for rip in range(ripetizioni):
            ...
            diz_rip[rip]=new_col
        return diz_rip
    def configuration(self,semp=True,remove=True):
        import random
    ...
    def set_RC(self):
        ...
        return prova
    def set_CM(self):
        test=self.configuration(semp=False) #FISSA UN CONFIGURATION MODEL
        return test
    def V_A_RC(self):
    ...
    def diff_mu_RC(self,ripetizioni=1000):
        val_att=self.V_A_RC()
    ...
    def tabella_err_RC(self,ripetizioni=1000):
        import matplotlib.pyplot as plt
        val_att=self.V_A_RC()
        media=self.mu_RC_montecarlo(ripetizioni=ripetizioni)#Tabella errori 
        err=dict()
        err_omo=dict()
        err_ete=dict()
        for a in val_att:
        ...
        dati=[]
        for a in val_att:
            dati.append([a,val_att[a],media[a],err[a]*100])
        ig, ax = plt.subplots()
        ax.axis('tight')
        ax.axis('off')
        table = ax.table(cellText=dati,
                 colLabels=['i', 'E_RC', 'Monte', 'ε%'],
                 cellLoc='center',
                 loc='center',
                 colWidths=[0.2,0.2,0.2,0.2])
        
        table.set_fontsize(40)
        plt.savefig('tabella_confronto.png', bbox_inches='tight', dpi=300)
        plt.show()
        return sum(err.values())/len(err.values())*100
    ...
    def diff_mu_CM(self,ripetizioni=1000): #DIFFERENZE VALORE ATTESO MENO MEDIE MONTECARLO PER IL CM
        ...
        diff_ete=dict()
        for a in val_att:
                diff[a]=(val_att[a]-media[a])/media[a]
                if a[0]==a[1]:
                    diff_omo[a]=(val_att[a]-media[a])/media[a]
                else:
                    diff_ete[a]=(val_att[a]-media[a])/media[a]
    ...
    def errore_mu_CM(self,relativo=False,ripetizioni=1000): #ERRORE CM
        ...
        for a in val_att:
            ...
            if a[0]==a[1]:
                ...
                err_ete[a]=valore
        return err,err_omo,err_ete
    def media_err_CM(self,ripetizioni=1000,omo_ete=0):      #MEDIA ERRORE RELATIVO CM
        err=self.errore_mu_CM(True,ripetizioni)[omo_ete]    #omo_ete : 0 errore rel tutti archi 1 archi omofili 2 archi eterofili
        media=sum(err.values())/len(err.values())
        return media
    
    def tabella_err_CM(self,ripetizioni=1000):
        import matplotlib.pyplot as plt
        val_att=self.V_A_CM()
        media=self.mu_CM_montecarlo(ripetizioni=ripetizioni)#Tabella errori 
        err=dict()
        err_omo=dict()
        err_ete=dict()
        for a in val_att:
        ...
        ax.axis('off')
        table = ax.table(cellText=dati,
                 colLabels=['i', 'E_CM', 'Monte', 'ε%'],
                 cellLoc='center',
                 loc='center')
        plt.savefig('tabella_confronto.png', bbox_inches='tight', dpi=300)
        plt.show()
        return sum(err.values())/len(err.values())*100
    def tabella_diff_CM(self,ripetizioni=1000):
        import matplotlib.pyplot as plt
        val_att=self.V_A_CM()
        media=self.mu_CM_montecarlo(ripetizioni=ripetizioni)#Tabella errori 
        diff=dict()
        for a in val_att:
            valore=(val_att[a]-media[a])/val_att[a]
            diff[a]=valore
        dati=[]
        for a in val_att:
            dati.append([a,val_att[a],media[a],diff[a]*100])
        ig, ax = plt.subplots()
        ax.axis('tight')
        ax.axis('off')
        table = ax.table(cellText=dati,
                 colLabels=['i', 'E_CM', 'Monte', '%'],
                 cellLoc='center',
                 loc='center')
        plt.savefig('tabella_confronto.png', bbox_inches='tight', dpi=300)
        plt.show()
        return sum(diff.values())/len(diff.values())*100
    ...
    def plot_err_rip_CM(self,omo_ete=0): #PLOT MEDIA ERRORE CM PER SIMULAZIONI CM
        import matplotlib.pyplot as plt
        rip = [50, 100, 200, 500, 1000,2000,5000,10000,20000,50000]

        medie = [self.media_err_CM(r,omo_ete) for r in rip]
        plt.figure(figsize=(7,5))
    ...
    def tabella_err_var_RC(self,ripetizioni=1000):
        import matplotlib.pyplot as plt
        err=dict()
        var=self.var_RC_esatta()
        monte=self.var_RC_montecarlo(ripetizioni)
        ...
        dati=[]
        for a in monte:
            dati.append([a,var[a],monte[a],err[a]*100])
        ig, ax = plt.subplots()
        ax.axis('tight')
        ax.axis('off')
        table = ax.table(cellText=dati,
                 colLabels=['ij', 'Var_RC', 'Monte', '%'],
                 cellLoc='center',
                 loc='center')
        plt.savefig('tabella_confronto.png', bbox_inches='tight', dpi=300)
        plt.show()
